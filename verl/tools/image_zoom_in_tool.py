...
class ImageZoomInTool(BaseTool):
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[ToolResponse, float, dict]:
        ...
        # Provide detailed guidance for multi-region exploration
        cropped_width, cropped_height = cropped_image.size
        # Get original image size from instance_data
        original_width, original_height = image.size
        response_text = (
            f"Successfully zoomed in on the region {bbox_2d} (magnified view: {cropped_width}×{cropped_height}px).\n\n"
            f"**IMPORTANT - Multi-Region Exploration Mode**:\n"
            f"• You are still viewing the ORIGINAL image (size: {original_width}×{original_height}px)\n"
            f"• The cropped region {bbox_2d} is now displayed with magnified details\n"
            f"• If you zoom again, use coordinates on the ORIGINAL {original_width}×{original_height}px image to explore a DIFFERENT region\n"
            f"• Valid coordinate range: [0, 0] to [{original_width}, {original_height}]\n\n"
            "Now analyze the magnified view following the three-step methodology:\n\n"
            "**Continue Step 3 - Detailed Analysis:**\n\n"
            "1. **Review Your Hypothesis**: Why did you select this region? What defect type were you investigating?\n\n"
            "2. **Texture Analysis**: Examine surface texture. Any irregular patterns, roughness, or discontinuities?\n\n"
            "3. **Edge Characteristics**: Inspect edges of any features:\n"
            "   - Sharp, well-defined edges → physical defects (cracks, holes, chips)\n"
            "   - Soft, diffuse edges → shadows, smudges, or normal variations\n\n"
            "4. **Depth and Dimensionality**: Look for depth variation (shadowing/gradients):\n"
            "   - 3D feature (hole, pit, protrusion) → likely defect\n"
            "   - Surface-level mark (shadow, reflection) → likely normal\n\n"
            "5. **Next Steps Decision**:\n"
            "   - If you have sufficient evidence → provide final answer (Format 2 or 3)\n"
            f"   - If you need to examine a DIFFERENT region → zoom to another area on the original {original_width}×{original_height}px image (max 2 zooms total)\n\n"
            "Remember: Each zoom explores an independent region on the original image. Use this to examine multiple suspicious areas."
        )
        if label:
            response_text = (
                f"Successfully zoomed in on the region {bbox_2d} labeled as '{label}' (magnified view: {cropped_width}×{cropped_height}px).\n\n"
                f"**IMPORTANT - Multi-Region Exploration Mode**:\n"
                f"• You are still viewing the ORIGINAL image (size: {original_width}×{original_height}px)\n"
                f"• The cropped region {bbox_2d} labeled '{label}' is now displayed with magnified details\n"
                f"• If you zoom again, use coordinates on the ORIGINAL {original_width}×{original_height}px image to explore a DIFFERENT region\n"
                f"• Valid coordinate range: [0, 0] to [{original_width}, {original_height}]\n\n"
                "Now analyze the magnified view following the three-step methodology:\n\n"
                "**Continue Step 3 - Detailed Analysis:**\n\n"
                f"1. **Review Your Hypothesis**: Why did you select region '{label}'? What defect type were you investigating?\n\n"
                "2. **Texture Analysis**: Examine surface texture. Any irregular patterns, roughness, or discontinuities?\n\n"
                "3. **Edge Characteristics**: Inspect edges of any features:\n"
                "   - Sharp, well-defined edges → physical defects (cracks, holes, chips)\n"
                "   - Soft, diffuse edges → shadows, smudges, or normal variations\n\n"
