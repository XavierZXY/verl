...
def load_dataset_with_masks(
    data_path: str = None, debug: bool = False, sample_size: int = 5
):
    """
    ...
    try:
        ...
        dataset_with_masks = dataset.filter(
            lambda x: "mask" in x and x["mask"] is not None
        )

        if debug:
            dataset_with_masks = dataset_with_masks.select(
                range(min(sample_size, len(dataset_with_masks)))
            )

        log.info(
            f"Dataset loaded. Found {len(dataset_with_masks)} samples with mask information."
        )
...
def enhance_dataset_with_bboxes(
    dataset, output_path: str = None, use_multiple_bboxes: bool = True
):
    """
    ...
    for i, sample in enumerate(dataset):
        log.info(
            f"Processing sample {i + 1}/{len(dataset)}: {sample.get('filename', 'unknown')}"
        )
        ...
        if use_multiple_bboxes:
            ...
            if bboxes is not None and len(bboxes) > 0:
                ...
                failed_count += 1
                log.warning(
                    f"Failed to calculate bboxes for: {sample.get('filename', 'unknown')}"
                )
        else:
            ...
            if bbox is not None:
                ...
                failed_count += 1
                log.warning(
                    f"Failed to calculate bbox for: {sample.get('filename', 'unknown')}"
                )

        enhanced_data.append(enhanced_sample)

    log.info(
        f"Bbox calculation completed. Success: {successful_count}, Failed: {failed_count}"
    )
...
def main():
    ...
    enhanced_data = enhance_dataset_with_bboxes(
        dataset, "enhanced_dataset_with_bboxes.jsonl"
    )
